...
export const removePoiShoppingLayer = (map: maplibregl.Map) => {
  const anyMap = map as any;

  // Remove all city shopping POI layers/sources
  ...
  if (anyMap._poiShoppingHoverEl) {
    anyMap._poiShoppingHoverEl.remove();
    anyMap._poiShoppingHoverEl = undefined;
  }

  anyMap._poiShoppingEventsBound = false;
};

export const ensurePoiShoppingLayer = (
  map: maplibregl.Map,
  city: CityId
) => {
  ...
  // 2) Circle layer – pink shopping dots
  ...
  if (!anyMap._poiShoppingEventsBound) {
    const container = map.getContainer();
    let hoverEl = anyMap._poiShoppingHoverEl as HTMLDivElement | undefined;

    if (!hoverEl) {
      hoverEl = document.createElement("div");
      hoverEl.className = "poi-shopping-hover-label";
      ...
      anyMap._poiShoppingHoverEl = hoverEl;
    }
    ...
    for (const c of CITY_LIST) {
      ...
      map.on("mousemove", lid, (e: MapLayerMouseEvent) => {
        ...
        const name =
          ...
          (props["shop_name"] as string | undefined)?.trim() ||
          (props["brand"] as string | undefined)?.trim() ||
          "Shopping location";
      ...
      map.on("click", lid, (e) => {
        ...
        console.log("Shopping POI click:", {
          city: c,
          ...feature.properties,
        });
      });
